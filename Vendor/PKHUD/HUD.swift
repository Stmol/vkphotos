...
public final class HUD {
    ...
    // MARK: Yury Smidovich
    // Худ с отменой
    public static func show(cancelHandler: (() -> Void)?, showCancelAfter: Double = 2.5, onView view: UIView? = nil) {
        self.allowsInteraction = false
        let contentView = PKHUDLoadingWithCancelView(delayBeforeShowCross: showCancelAfter) {
             PKHUD.sharedHUD.hide(animated: false, completion: { _ in
                 PKHUD.sharedHUD.contentView = UIView() // TODO: Это чтобы contentView не висел после cancel
             })
        }

        contentView.onCancelCrossTap = cancelHandler
    ...
    fileprivate static func contentView(_ content: HUDContentType) -> UIView {
        ...
        case .loadingWithCancel, .loading:
            return PKHUDLoadingView()
